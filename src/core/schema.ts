...
const marks: { [name: string]: MarkSpec } = {
  ...
  backgroundColor: {
    ...
    parseDOM: [
      ...
      {
        tag: 'mark',
        getAttrs: (node: any) => ({ color: node.style.backgroundColor || '#ffff00' })
      }
    ],
    toDOM(mark) {
      const { color } = mark.attrs
...
nodes.image = {
    ...
    },
    {
      ...
      getAttrs: (dom: any) => {
        ...
        return {
          ...
          alignment: wrapper?.classList.contains('image-left') ? 'left' :
                    wrapper?.classList.contains('image-right') ? 'right' : 'center'
        }
      }
    }
  ],
  toDOM(node) {
    ...
    return ['div', { class: `image-wrapper image-${alignment}` }, ['img', imgAttrs]]
  }
}
