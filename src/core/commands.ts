...
export function createCommands(editor: Editor): SingleCommands {
  return {
    ...
    underline: () =>
      executeCommand(editor, toggleMark(editor.schema.marks.underline)),
    strike: () =>
      executeCommand(editor, toggleMark(editor.schema.marks.strikethrough)),
    code: () => executeCommand(editor, toggleMark(editor.schema.marks.code)),
    superscript: () =>
      executeCommand(editor, toggleMark(editor.schema.marks.superscript)),
    subscript: () =>
      executeCommand(editor, toggleMark(editor.schema.marks.subscript)),
    ...
    textColor: (color: string | null) => {
      ...
      if (color === null) {
        tr.removeMark(
          tr.selection.from,
          tr.selection.to,
          editor.schema.marks.textColor,
        );
      } else {
        tr.addMark(
          tr.selection.from,
          tr.selection.to,
          editor.schema.marks.textColor.create({ color }),
        );
      }
    ...
    backgroundColor: (color: string | null) => {
      ...
      if (color === null) {
        ...
        tr.addMark(
          tr.selection.from,
          tr.selection.to,
          editor.schema.marks.backgroundColor.create({ color }),
        );
      }
      editor.view.dispatch(tr);
      return true;
    ...
    fontFamily: (family: string | null) => {
      ...
      if (family === null) {
        tr.removeMark(
          tr.selection.from,
          tr.selection.to,
          editor.schema.marks.fontFamily,
        );
      } else {
        tr.addMark(
          tr.selection.from,
          tr.selection.to,
          editor.schema.marks.fontFamily.create({ family }),
        );
      }
      editor.view.dispatch(tr);
      return true;
    ...
    fontSize: (size: string | null) => {
      ...
      if (size === null) {
        tr.removeMark(
          tr.selection.from,
          tr.selection.to,
          editor.schema.marks.fontSize,
        );
      } else {
        tr.addMark(
          tr.selection.from,
          tr.selection.to,
          editor.schema.marks.fontSize.create({ size }),
        );
      }
      editor.view.dispatch(tr);
      return true;
    ...
    // Block formatting
    paragraph: () =>
      executeCommand(editor, setBlockType(editor.schema.nodes.paragraph)),
    heading: (level: 1 | 2 | 3 | 4 | 5 | 6 = 1) => {
      const headingNode =
        editor.schema.nodes[`heading${level}`] || editor.schema.nodes.heading;
      return executeCommand(editor, setBlockType(headingNode, { level }));
    },
    blockquote: () =>
      executeCommand(editor, wrapIn(editor.schema.nodes.blockquote)),
    codeBlock: () =>
      executeCommand(editor, setBlockType(editor.schema.nodes.code_block)),

    // Lists
    bulletList: () =>
      executeCommand(editor, wrapInList(editor.schema.nodes.bullet_list)),
    orderedList: () =>
      executeCommand(editor, wrapInList(editor.schema.nodes.ordered_list)),
    listItem: () =>
      executeCommand(editor, splitListItem(editor.schema.nodes.list_item)),
    liftListItem: () =>
      executeCommand(editor, liftListItem(editor.schema.nodes.list_item)),
    sinkListItem: () =>
      executeCommand(editor, sinkListItem(editor.schema.nodes.list_item)),
      ...
      editor.state.doc.descendants((node, pos) => {
        if (node.type.name === "image" && node.attrs.src === oldSrc) {
          const newAttrs = {
            ...
            alignment:
              alignment !== undefined ? alignment : node.attrs.alignment,
          };
