<template>
  <div class="editor-toolbar">
    <template v-for="(item, _index) in items" :key="_index">
      ...
      <div v-else-if="item.type === 'group'" class="editor-toolbar__group">
        <EditorToolbar v-if="item.items" :editor="editor" :items="item.items" />
      </div>
...
const getCurrentColor = (item: ToolbarItem): string | undefined => {
  if (item.command === "textColor") {
    ...
    const mark = props.editor.state.selection.$from
      .marks()
      .find((m: any) => m.type.name === "textColor");
  ...
  if (item.command === "backgroundColor") {
    ...
    const mark = props.editor.state.selection.$from
      .marks()
      .find((m: any) => m.type.name === "backgroundColor");
    return mark?.attrs.color || undefined;
  }
  return undefined;
};
...
const getCurrentFont = (item: ToolbarItem): string | undefined => {
  if (item.command === "fontFamily") {
    const mark = props.editor.state.selection.$from
      .marks()
      .find((m: any) => m.type.name === "fontFamily");
    return mark?.attrs.family || undefined;
  }
  if (item.command === "fontSize") {
    const mark = props.editor.state.selection.$from
      .marks()
      .find((m: any) => m.type.name === "fontSize");
...
const handleLinkInsert = (data: {
  ...
  if (data.text && !props.editor.state.selection.empty) {
    ...
    tr.replaceSelectionWith(
      props.editor.schema.text(data.text, [
        linkMark.create({
          href: data.url,
          title: data.title,
          target: data.target,
